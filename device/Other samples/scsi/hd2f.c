...
main()
{
    ...
    if(fd<0) printf("Unable to open input file.\n");
    else {
        if(sizeof(header)!=read(fd,&header,sizeof(header)))
            printf("Error reading header block.\n");
        else if(header.sig1 != FILE_SIG || header.sig2 != ~FILE_SIG)
            printf("Not a disk dump.\n");
        else {
            printf("Extracting: %s\n",header.name);
            ...
            for(i=0;i<80;i+=NUMTRKS) {
                printf("Reading...              \r");
                fflush(stdout);
                if(TD_BUFSIZE != read(fd,diskbuf,TD_BUFSIZE)) {
                    printf("Error reading from file.\n");
                    break;
                } else {
                    printf("Writing tracks %ld to %ld.\r",i,i+NUMTRKS-1);
                    fflush(stdout);
