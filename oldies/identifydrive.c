...
#define IDE_BASE_ADDRESS (0xF20000)
...
main (int argc, char * argv[])
{
   ...
   TF_DRIVE_HEAD=DRV0;
   TF_DEVICE_CONTROL=8+nIEN+SRST;   /* Soft Reset + No Interrupt Request*/
   ...
   while (((TF_STATUS&(BSY|DRDY|SKC|DRQ))!=(DRDY|SKC|DRQ))&&time)
      {
         unsigned char erroria;

         time--;
         erroria = TF_STATUS&(ERR|DWF);
         if (erroria) if (time > 100) time = time - 100;
         if (erroria) puts ("Error or Drive Write Fault");
